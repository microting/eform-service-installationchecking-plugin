...
if (( "$GIT_STATUS" > 0 )); then
	git checkout stable
	git pull
	cd ServiceInstallationCheckingPlugin

	dotnet add ServiceInstallationCheckingPlugin.csproj package Microting.eForm
	dotnet add ServiceInstallationCheckingPlugin.csproj package Microting.InstallationCheckingBase
